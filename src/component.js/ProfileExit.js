...
const ProfileExit = ({userdata,darktheme}) => {
    ...
    console.log(userdata.created_at.split("T"))
    return (
        <>
            <div className='msy-img-cont col-lg-4 p-3'>
                <img src={userdata.avatar_url} className='w-100 rounded-circle' />
            </div>
            <div className='col-lg-8 p-5'>
                <div className='d-flex justify-content-between'>
                    <h2>{userdata.name}</h2>
                    <p className='pt-3'>{day+"/"+month+"/"+year}</p>
                </div>
                <div className='mb-5'>

                    <p className='pt-3' style={{ color: "8px" }}>{userdata.bio == null ? "This profile has no bio" : userdata.bio}</p>
                </div>
                <div className={`d-flex justify-content-between flex-row  p-3 rounded-lg  ${darktheme?"msy-folowerDark":"msy-folowerlight"}`}>
                    <div>
                        <h5>Repos</h5>
